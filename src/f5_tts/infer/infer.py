...
with torch.inference_mode():
    ...
    generated2, _ = ema_model.sample_streaming(
        cond=ref,
        text=token,
        duration=ref_audio_len,
        steps=nfe_step,
    ...
    gen_mel_spec2 = generated2[:, :, :].permute(0, 2, 1).cpu()
    ...
    # 保存为 .npy 文件
    np.save(output_path_gen, gen_mel_spec.numpy())
    np.save(output_path_gen2, gen_mel_spec2.numpy())
    with torch.no_grad():
        decode_mel(gen_mel_spec,f"/home/node57_data/hkxie/4O/F5-TTS/src/f5_tts/infer/test/streaming_gen400k_test_final_nostreaming.wav")
