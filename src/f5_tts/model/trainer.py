...
# from third_party.dspgan.test_vocoder import decode_mel
# trainer
...
class Trainer:
    ...
    def train(self, train_dataset: Dataset, num_workers=16, resumable_with_seed: int = None):
        if self.log_samples:
            ...
            # vocoder = load_vocoder( #Download Vocos from huggingface charactr/vocos-mel-24khz
            ...
            for batch in current_dataloader:
                with self.accelerator.accumulate(self.model):
                    ...
                    loss, cond, pred = self.model(
                        mel_spec, text=text_inputs, lens=mel_lengths, noise_scheduler=self.noise_scheduler,ref_embed = ref_embd, spk_emb = spk_emb
                    )
                ...
                if global_update % self.save_per_updates == 0 and self.accelerator.sync_gradients:
                    ...
                    #         else:
                    #             with torch.no_grad():
                    #                 decode_mel(gen_mel_spec,f"{log_samples_path}/update_{global_update}_gen.wav")
