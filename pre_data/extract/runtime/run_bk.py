...
class VCRunner():
    def __init__(self, spkid):
        ...
        self.vc = torch.jit.load('runtime/models/vc_fuxiall.pt')
        self.vocoder = torch.jit.load('runtime/models/refineganft.pt')
        ...
        self.spks = torch.from_numpy(np.load('zimu.npy')).unsqueeze(0)
    ...
    def playback(self):
        p = pyaudio.PyAudio()
        self.out_stream = p.open(format=pyaudio.paFloat32, channels=1, rate=16000, output=True)
        #self.out_stream = p.open(format=pyaudio.paFloat32, channels=1, rate=16000, output=True, output_device_index=12)
        while True:
            #print("running...")
            data = self.in_stream.read(self.CHUNK, exception_on_overflow = False)
            samples = np.fromstring(data, dtype='float32')
