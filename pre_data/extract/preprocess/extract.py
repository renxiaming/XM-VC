    ...
    if not use_lance:
        ...
        if filelist is not None:
            ...
            with open(filelist, "r") as f:  
                generator = [line.strip().split(" ") for line in f if line.strip()]
        else:
            generator = os.listdir(data_root)

        for utt, wav_filename in tqdm(generator):
            ...
            try:
                ...
                print("!!!!!!!", wav_path)
            np.save(os.path.join(out_data_root, os.path.basename(wav_filename).split('.')[0]), bn)
