...
def extract_bn(wav_path, use_lance=False):
    ...
    for i in range(0, fbanks.shape[1], 20):
        ...
        encoder_output_upsample = torch.nn.functional.interpolate(encoder_output_upsample, size=int(encoder_output_upsample.shape[2] * 4), mode='linear', align_corners=True)
...
def extract_bn_lance(row, reader:LanceReader, already_dataid):
    # import pdb;pdb.set_trace()
    try:
        item = reader.get_datas_by_rowids([row])[0]
        if item.data_id in already_dataid:
            return None
        bn = extract_bn(item, use_lance=True)
        # bn = extract_bn(reader.get_datas_by_rowids([row])[0], use_lance=True)
...
if __name__ == "__main__":
    ...
    if not use_lance:
        ...
        # rowids = [wav._rowid for wav in wav_ids]
        # rows = list(range(len(wav_ids)))

        # 只打包还没有保存的数据
        wav_ids = wav_reader.get_ids()
        wav_data_ids = [wav.data_id for wav in wav_ids]
        writer_ids = writer.get_ids()
        writer_data_ids = [wav.data_id for wav in writer_ids]
        intersection = set(wav_data_ids) & set(writer_data_ids)
        already_dataid = list(intersection)
        print("the length already_dataid:", len(already_dataid))
        ...
        # import pdb;pdb.set_trace()
        print("device:", device)
        bn_function = partial(extract_bn_lance, reader=wav_reader, already_dataid=already_dataid)
