...
class BaseEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        if not self.two_chunk:
            #for i in range(self.s_ctc_layer - self.add_l_num_blocks):
            for layer in self.encoders[:7]:
                #s_xs, s_chunk_masks, _, _ = self.encoders[i](s_xs, s_chunk_masks, pos_emb, mask_pad)
    ...
    def forward_chunk(
        ...
        for i, layer in enumerate(self.encoders):
            if i < self.s_ctc_layer - self.add_l_num_blocks:
                #print(i)
                _ = i
            else:
                #print(self.s_ctc_layer - self.add_l_num_blocks)
                break
